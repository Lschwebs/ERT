...
function data = preprocLipp(dataLoc, minVal, errRecip)

%% import file and create data matrices
D = importLippmann(dataLoc); % load raw data array
abmn = [D(:,2) D(:,3) D(:,4) D(:,5)]; % takes electrode locations from raw data
R = D(:,8); % takes resistance from raw data
dat = [abmn R]; % makes a single matrix out of all raw data (NO ERRORS RIGHT NOW)
...
for i = 1:length(RECIPS)
    ...
    if RECIPS(i,10) < errRecip % reciprocal error
        DAT(cnt,:) = [RECIPS(i,1:4), mean(RECIPS(i,5:6)), RECIPS(i,10)]; % loop through and keep all columns in each row below threshold
        cnt = cnt+1;
    end
end
...
protocolData = out;
newfile = [pwd '/protocol.dat']; % create protocal.dat file
dlmwrite(newfile, mn) % write first line of protocal.dat
dlmwrite(newfile, protocolData, '-append','delimiter','\t') % write line 2-mn filtered data to protocal.dat
clear newfile;
