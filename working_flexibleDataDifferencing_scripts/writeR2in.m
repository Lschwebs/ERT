...
num_regions = number of resistivity regions for starting model

if num_regions = 0:
    startRes = resistivity file from previous inversion
else:
    startRes = geometric mean of apparent resistivities
end
...
num_electrodes  number of elecrodes in survey
...
elecs = [elecs (elecs+1)]; % electrode, node
...
if num_regions == 0
    line12 = startRes; % starting model for TIME_LAPSE inversion (reg_mode = 2)
else
    line12 = [1 numel startRes]; % starting resistivity value (geometric mean of app res)
end
...
if num_regions == 0
    dlmwrite(newfile, line12, '-append', 'delimiter', '')
else
    dlmwrite(newfile, line12, '-append', 'delimiter', ' ')
end
