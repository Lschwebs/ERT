...
R_squared = 1 - (gof.normr/norm(lbins(:,2) - mean(lbins(:,2))))^2;

%% plot and save
stats = [10.^P(2) P(1) R_squared]; % a b R2 value where ax^b
err = 10.^P(2) .* bins(:, 1).^P(1);
...
xlabel('R_{avg} (\Omega)')
ylabel('R_{err} (\Omega)')
title(['R_{err} = ', num2str(10.^P(2)) ,'*R_{avg}**', num2str(P(1))], [' R2 = ', num2str(R_squared)])
hold off;
